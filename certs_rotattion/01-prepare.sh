...
K8S_APISERVER_SANS=$(extract_sans_from_cert "${FIRST_MASTER_HOSTNAME}/old/kubernetes/pki/apiserver.crt")
ETCD_ADMIN_SANS=$(extract_sans_from_cert "${FIRST_ETCD_HOSTNAME}/old/etcd-ssl/admin-${FIRST_ETCD_HOSTNAME}.pem")
ETCD_NODE_SANS=$(extract_sans_from_cert "${FIRST_ETCD_HOSTNAME}/old/etcd-ssl/node-${FIRST_ETCD_HOSTNAME}.pem")
ETCD_MEMBER_SANS=$(extract_sans_from_cert "${FIRST_ETCD_HOSTNAME}/old/etcd-ssl/member-${FIRST_ETCD_HOSTNAME}.pem")

log "提取到 K8s APIServer SANs: ${K8S_APISERVER_SANS}"
log "提取到 ETCD Admin SANs:   ${ETCD_ADMIN_SANS}"
log "提取到 ETCD Node SANs:    ${ETCD_NODE_SANS}"
log "提取到 ETCD Member SANs:  ${ETCD_MEMBER_SANS}"
...
for node_ip in "${ALL_NODES[@]}"; do
    ...
    if [[ " ${MASTER_NODES[@]} " =~ " ${node_ip} " ]] || [[ " ${ETCD_NODES[@]} " =~ " ${node_ip} " ]]; then
        ...
        generate_leaf_cert new-cas/etcd/ca.pem new-cas/etcd/ca-key.pem \
            ...
            "/CN=etcd-admin-${hostname}" "${ETCD_ADMIN_SANS}" "serverAuth,clientAuth"
        generate_leaf_cert new-cas/etcd/ca.pem new-cas/etcd/ca-key.pem \
            "${hostname}/new/etcd-ssl/node-${hostname}-key.pem" "${hostname}/new/etcd-ssl/node-${hostname}.pem" \
            "/CN=etcd-node-${hostname}" "${ETCD_NODE_SANS}" "serverAuth,clientAuth"
        generate_leaf_cert new-cas/etcd/ca.pem new-cas/etcd/ca-key.pem \
            "${hostname}/new/etcd-ssl/member-${hostname}-key.pem" "${hostname}/new/etcd-ssl/member-${hostname}.pem" \
            "/CN=etcd-member-${hostname}" "${ETCD_MEMBER_SANS}" "serverAuth,clientAuth"
    fi
done
...
for node_ip in "${ALL_NODES[@]}"; do
    ...
    mkdir -p "${hostname}/bundle/kubernetes/pki/"
   ...
   if [[ " ${MASTER_NODES[@]} " =~ " ${node_ip} " ]] || [[ " ${ETCD_NODES[@]} " =~ " ${node_ip} " ]]; then
        if [ -d "${hostname}/bundle/etcd-ssl" ]; then
