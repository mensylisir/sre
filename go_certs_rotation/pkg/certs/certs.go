package certs
import ("crypto/rand"; "crypto/rsa"; "crypto/x509"; "crypto/x509/pkix"; "encoding/pem"; "fmt"; "math/big"; "net"; "strings"; "time")
...
func GenerateCertificate(spec CertSpec, caCertPEM, caKeyPEM []byte) (certPEM, keyPEM []byte, err error) {
	...
	for _, san := range spec.SANs {
		if ip := net.ParseIP(san); ip != nil { template.IPAddresses = append(template.IPAddresses, ip) } else { template.DNSNames = append(template.DNSNames, san) }
	}
	if spec.IsCA {
		template.IsCA = true
...
func ParseSubject(subj string) (pkix.Name, error) {
	...
	if name.CommonName == "" { return name, fmt.Errorf("subject must contain a Common Name (CN)") }
	return name, nil
}
